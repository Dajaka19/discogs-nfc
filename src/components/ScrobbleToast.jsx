...
export function Disc({ kind, color, translucent, size = 84, image, labelColor, imageZoom = 1 }) {
  ...
  if (image) {
    ...
    const z = Math.round(100 * (imageZoom || 1))
    return (
        ...
        >
          ...
          <div
            ...
            style={{
              width: hub,
              height: hub,
              // Explicit label colour wins; otherwise coloured vinyl → dark label
              // for contrast, black vinyl → the app accent.
              background: labelColor || (color ? '#0e0e12' : '#f5a623'),
...
export default function ScrobbleToast({ kind = 'cd', color, translucent, image, labelColor, imageZoom, count = 0, partial, onDone }) {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center px-4 pointer-events-none">
      ...
      >
        <div className="relative shrink-0">
          ...
          />
          <Disc kind={kind} color={color} translucent={translucent} image={image} labelColor={labelColor} imageZoom={imageZoom} />
          {/* success check badge */}
